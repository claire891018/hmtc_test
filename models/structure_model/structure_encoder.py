...
from models.structure_model.graphcnn import HierarchyGCN, HierarchyGAT
...
class StructureEncoder(nn.Module):
    def __init__(self,
        ...
        :param graph_model_type: Str, model_type, ['TreeLSTM', 'GCN', 'GAT']
        """
        ...
        if graph_model_type == 'GAT':
            common_kwargs.update(
                num_heads=getattr(config.structure_encoder, 'num_heads', 1),
                attn_dropout=getattr(config.structure_encoder, 'attn_dropout', None),
            )

        self.model = MODEL_MODULE[graph_model_type](**common_kwargs)
